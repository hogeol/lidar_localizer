...
namespace ImuProcessing{
  void imuProcessing::init(const int &imu_window_size, const int &gps_window_size)
  {
    positionClear();
  ...
  void imuProcessing::weightPrevPosition(const Eigen::Vector3d &pres_position)
  {
  ...
  void imuProcessing::positionClear()
  {
  ...
  int imuProcessing::getImuWindowSize() const
  {
