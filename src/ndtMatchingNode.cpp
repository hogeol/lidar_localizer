...
int main(int argc, char** argv)
{
  ...
  auto nh{std::make_shared<rclcpp::Node>("ndt_matching_node")};
  
  nh -> declare_parameter("pcd_map_path", "/home/a/ace_ws/src/velodyne_ndt/map/");
  std::string pcd_map_path(nh->get_parameter("pcd_map_path").as_string());
  nh -> declare_parameter("pcd_map_name", "map.pcd");
  std::string pcd_map_name(nh->get_parameter("pcd_map_name").as_string());
  nh -> declare_parameter("pcd_map_resolution", 2.0);
  double pcd_map_resolution(nh->get_parameter("pcd_map_resolution").as_double());
  nh -> declare_parameter("map_translation_x", 0.0);
  double map_translation_x(nh->get_parameter("map_translation_x").as_double());
  nh -> declare_parameter("map_translation_y", 0.0);
  double map_translation_y(nh->get_parameter("map_translation_y").as_double());
  nh -> declare_parameter("map_translation_z", 0.0);
  double map_translation_z(nh->get_parameter("map_translation_z").as_double());
  nh -> declare_parameter("map_rotation_theta", 0.0);
  double map_rotation_theta(nh->get_parameter("map_rotation_theta").as_double());
  nh -> declare_parameter("user_init_pose", false);
  bool user_init_pose(nh->get_parameter("user_init_pose").as_bool());
  nh -> declare_parameter("odom_init_x", 0.0);
  double odom_init_x(nh->get_parameter("odom_init_x").as_double());
  nh -> declare_parameter("odom_init_y", 0.0);
  double odom_init_y(nh->get_parameter("odom_init_y").as_double());
  nh -> declare_parameter("odom_init_z", 0.0);
  double odom_init_z(nh->get_parameter("odom_init_z").as_double());
  nh -> declare_parameter("odom_init_rotation", 0.0);
  double odom_init_rotation(nh->get_parameter("odom_init_rotation").as_double());
  nh -> declare_parameter("sensor_diff_x", 0.0);
  double sensor_diff_x(nh->get_parameter("sensor_diff_x").as_double());
  nh -> declare_parameter("sensor_diff_y", 0.0);
  double sensor_diff_y(nh->get_parameter("sensor_diff_y").as_double());
  nh -> declare_parameter("sensor_diff_z", 0.0);
  double sensor_diff_z(nh->get_parameter("sensor_diff_z").as_double());
  nh -> declare_parameter("submap_select", false);
  bool submap_select(nh->get_parameter("submap_select").as_bool());
  ...
  ndt_matching.init(pcd_map_resolution, pcd_map_path, pcd_map_name, submap_select, kdtree_search_radius, ndt_near_points, ndt_max_iteration, ndt_max_threads);
